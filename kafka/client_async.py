    ...
    def _bootstrap(self, hosts):
        ...
        if self.config['api_version'] is None or self.config['api_version'] < (0, 10):
            if self.config['topics']:
                metadata_request = MetadataRequest[0](list(self.config['topics']))
            else:
                metadata_request = MetadataRequest[0]([])
        else:
            if self.config['topics']:
                metadata_request = MetadataRequest[1](list(self.config['topics']))
            else:
    ...
    def check_version(self, node_id=None, timeout=2, strict=False):
        ...
        while time.time() < end:
            ...
            try:
                ...
                version = conn.check_version(timeout=remaining, strict=strict, topics=list(self.config['topics']))
                return version
