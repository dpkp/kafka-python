...
MAX_PARTITION_ALLOCATION_TIME = 360  # in seconds
...
class ZProducer(object):
    ...
    def __init__(self, hosts, topic, chroot='/', **kwargs):
        ...
        if self.producer_kls is None:
            raise NotImplementedError("Producer class needs to be mentioned")
...
class ZKeyedProducer(ZProducer):
    ...
    def send(self, key, msg):
        self.producer.send_messages(key, msg)
...
class ZSimpleConsumer(object):
    ...
    * After re-balancing, if the consumer does not get any partitions,
    ...
    def __init__(self,
        ...
        # Start the worker
        self.partioner_thread = threading.Thread(
            target=self._check_and_allocate)
    ...
    def _check_and_allocate(self):
        ...
        while not self.exit.is_set():
            log.info("ZK Partitoner state: %s, topic: %s, group: %s" % (
                partitioner.state, self.consumer_topic, self.consumer_group))
            try:
                if partitioner.acquired:
                    ...
                    if new != old:
                        ...
                        if len(new) > 0:
                            ...
                            self.consumer.register_on_stop_callback(
                    ...
                    # see:
                    # http://kazoo.readthedocs.org/en/latest/api/recipe/partitioner.html
                    ...
                    log.info(
                        "Waiting for partition allocation, topic: {0}, group: {1}, count: {2}, time: {3}".format(
                            self.consumer_topic,
                            self.consumer_group,
                            partition_allocation_attempt,
                            partition_allocation_end_time -
                            partition_allocation_start_time))
                    if partition_allocation_attempt > MAX_PARTITION_ALLOCATION_ATTEMPTS or \
                       partition_allocation_end_time - partition_allocation_start_time > MAX_PARTITION_ALLOCATION_TIME:
                        ...
                        log.info('Creating new partitioner, as the old one was in ALLOCATING state for too many attempts, topic: {0}, group: {1}, count: {2}, time: {3}'.format(
                            self.consumer_topic, self.consumer_group, partition_allocation_attempt, partition_allocation_end_time - partition_allocation_start_time))
                        partition_allocation_attempt = 0
                        partition_allocation_start_time = time.time()
                ...
                log.error(
                    "Exception raised in partitioner thread. Error:%s" %
                    e)
                self.error = e
                self.got_error = True
                break
