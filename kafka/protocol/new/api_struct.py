...
class ApiStruct:
    ...
    def to_schema(self, version, compact=False, tagged=False):
        ...
        if tagged:
            version_fields.append(('tags', self._tagged_fields(version)))
        return Schema(*[field for field in version_fields if field is not None])

    def _tagged_fields(self, version):
    ...
    def encode(self, item, version=None, compact=False, tagged=False):
        ...
        if tagged:
            ...
            encoded.append(self._tagged_fields(version).encode(tags, version=version,
                                                               compact=True, tagged=False))
        return b''.join(encoded)

    def decode(self, data, version=None, compact=False, tagged=False, data_class=None):
        # TODO: assert version is not None or isinstance(self, ApiMessage), 'version is required to decode non-schema structs'
        ...
        if tagged:
            decoded.update(self._tagged_fields(version).decode(data, version=version, compact=True, tagged=False))
